...
public class DisableUserCommandHandler(
    ...
    IKeycloakService keycloakService) : IRequestHandler<DisableUserCommand, ServiceResult>
{
    public async Task<ServiceResult> Handle(DisableUserCommand request, CancellationToken cancellationToken)
    {
        ...
        await keycloakService.DisableUserAsync(tenant.RealmName, request.UserId, cancellationToken);
